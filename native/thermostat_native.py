...
class NativeThermostatApp:
    ...
    def button(self, x1: int, y1: int, x2: int, y2: int, label: str, action: Callable[[], None], fill: str = PANEL_2, outline: str = CARD_BORDER, text: str = TEXT, tag: str = "", size: int = 18, radius: int = 16, border: int = 1) -> None:
        ...
        self.canvas.create_line(x1 + r, y1 + 2, x2 - r, y1 + 2, fill="#516070", width=1)
    ...
    def glass_panel(self, x1: int, y1: int, x2: int, y2: int, r: int = 22, fill: str = "#0d1626", outline: str = CARD_BORDER) -> None:
        ...
        self.round_rect(x1 + 1, y1 + 1, x2 - 1, y2 - 1, max(2, r - 1), "", "#273241", 1)
        self.canvas.create_line(x1 + r, y1 + 3, x2 - r, y1 + 3, fill="#5a6978", width=1)
    ...
    def glossy_button(self, x1: int, y1: int, x2: int, y2: int, label: str, action: Callable[[], None], active: bool = False, fill: str | None = None, text: str | None = None, outline: str | None = None, size: int = 11, tag: str = "") -> None:
        if fill is None:
    ...
    def draw_background(self) -> None:
        ...
        for x in range(0, self.width + grid, grid):
            self.canvas.create_line(x, 0, x, self.height, fill="#111a26", width=1)
        for y in range(0, self.height + grid, grid):
            self.canvas.create_line(0, y, self.width, y, fill="#101822", width=1)
        # Outer shell, with the same glass-panel framing as the web app.
        self.round_rect_shadow(self.sx(12), self.sy(14), self.sx(1268), self.sy(786), self.sx(34), "#06101d", "#4b5968", 1, shadow="#000000", offset=self.sy(5))
        self.round_rect(self.sx(16), self.sy(18), self.sx(1264), self.sy(782), self.sx(30), "", "#222c38", 1)
    ...
    def circle_button(self, cx:int, cy:int, r:int, label:str, action:Callable[[], None], fill:str="#111b2b", outline:str=CARD_BORDER, color:str=TEXT, size:int=20) -> None:
        ...
        self.canvas.create_arc(x1+self.sx(7), y1+self.sy(5), x2-self.sx(7), y2-self.sy(5), start=55, extent=80, style="arc", outline="#637282", width=1)
    ...
    def draw_header(self) -> None:
        ...
        self.canvas.create_line(x1+self.sx(16), y1+2, x2-self.sx(16), y1+2, fill="#4b5968", width=1)
        self.text((x1+x2)//2, (y1+y2)//2, label, 9, color, "bold")
        self.pill(self.sx(1092), self.sy(28), self.sx(1180), self.sy(72), time.strftime("%I:%M %p").lstrip("0"), fill="#101b2a", outline="#3a4654", color=TEXT, size=9)
        self.circle_button(1214, 50, 21, "i", lambda: self.open_info(), fill="#102f46", outline="#3bdfff", color=CYAN_2, size=17)
        self.circle_button(1252, 50, 21, "⚙", lambda: self.open_settings(), fill="#101b2a", outline="#3a4654", color="#dce7f5", size=14)
    ...
    def draw_top_nav(self) -> None:
        ...
        x = (1280 - total) // 2
        y, h = 28, 44
        self.round_rect_shadow(self.sx(x-8), self.sy(y-7), self.sx(x+total+8), self.sy(y+h+7), self.sy(28), "#0f1928", "#435160", 1, shadow="#02050a", offset=self.sy(3))
        self.canvas.create_line(self.sx(x+18), self.sy(y-4), self.sx(x+total-18), self.sy(y-4), fill="#546373", width=1)
        for page, label in pages:
            w = widths[page]
            active = self.page == page
            x1, y1, x2, y2 = self.sx(x), self.sy(y), self.sx(x+w), self.sy(y+h)
            fill = CYAN if active else "#0f1928"
            ...
            if active:
                ...
                self.canvas.create_line(x1+self.sx(18), y1+2, x2-self.sx(18), y1+2, fill="#b8c8d8", width=1)
    ...
    def draw_thermostat(self) -> None:
        ...
        self.pill(self.sx(540), self.sy(122), self.sx(740), self.sy(154), action_label, fill="#111b2b", outline="#435160", color=TEXT, size=10, dot=action_color)
        self.draw_web_style_dial(current, target, action_label, action_color)

        self.circle_button(376, 446, 37, "−", lambda: self.change_target(-1), fill="#111b2b", outline="#3a4654", color=TEXT, size=30)
        self.circle_button(904, 446, 37, "+", lambda: self.change_target(1), fill="#111b2b", outline="#3a4654", color=TEXT, size=29)
    ...
    def draw_web_style_dial(self, current: float, target: float, action_label: str, action_color: str) -> None:
        ...
        # Outer glass layers.
        self.canvas.create_oval(cx-r-30, cy-r-30, cx+r+30, cy+r+30, fill="#02050a", outline="#111a26", width=1)
        self.canvas.create_oval(cx-r-20, cy-r-20, cx+r+20, cy+r+20, fill="#07101d", outline="#3a4654", width=2)
        self.canvas.create_oval(cx-r-7, cy-r-7, cx+r+7, cy+r+7, fill="#0a1422", outline="#333f4d", width=1)
        ...
        self.canvas.create_oval(cx-inner-16, cy-inner-16, cx+inner+16, cy+inner+16, fill=inner_dark, outline="#273241", width=1)
        self.canvas.create_oval(cx-inner, cy-inner, cx+inner, cy+inner, fill=inner_fill, outline=pill_outline, width=2)
        self.canvas.create_arc(cx-inner+9, cy-inner+9, cx+inner-9, cy+inner-9, start=52, extent=76, style="arc", outline="#b8c8d8", width=3)
        ...
        self.pill(cx-self.sx(160), cy+self.sy(146), cx-self.sx(110), cy+self.sy(172), f"{minimum:.0f}°", fill="#050a12", outline="#222c38", color=TEXT, size=10)
        self.pill(cx+self.sx(110), cy+self.sy(146), cx+self.sx(160), cy+self.sy(172), f"{maximum:.0f}°", fill="#050a12", outline="#222c38", color=TEXT, size=10)
